	...
	public String setsIsPostbackAttributeTest(TestBean testBean) {
		...
		if (BridgeUtil.getPortletRequestPhase(facesContext) == Bridge.PortletPhase.ACTION_PHASE) {
			...
			Boolean isPostback = (Boolean) facesContext.getExternalContext().getRequestMap().get(
					"com.liferay.portlet.faces.isPostback");

			if (isPostback == null) {
				testBean.setTestResult(false,
					"Didn't set the com.liferay.portlet.faces.isPostback attribute in render request following an action (Postback).");
				...
				if (isPostback.booleanValue()) {
					testBean.setTestResult(true,
						"Correctly set the com.liferay.portlet.faces.isPostback attribute as TRUE in render request following an action (Postback).");

					return Constants.TEST_SUCCESS;
				}
				else {
					testBean.setTestResult(false,
						"Incorrectly set the com.liferay.portlet.faces.isPostback attribute as FALSE in render request following an action (Postback).");
