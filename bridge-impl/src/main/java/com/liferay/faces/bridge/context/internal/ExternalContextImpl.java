...
import com.liferay.portlet.faces.BridgeDefaultViewNotSpecifiedException;
import com.liferay.portlet.faces.BridgeException;
import com.liferay.portlet.faces.BridgeFactoryFinder;
import com.liferay.portlet.faces.BridgeInvalidViewPathException;
import com.liferay.portlet.faces.BridgeURL;
...
public class ExternalContextImpl extends ExternalContextCompat_Portlet3_Impl {
	...
	public void redirect(String url) throws IOException {
		...
		if (url != null) {
			...
			if ((portletPhase == Bridge.PortletPhase.ACTION_PHASE) ||
				...
				// "com.liferay.portlet.faces.DirectLink" parameter value of "true", then
				try {
		...
		if (response != null) {
			...
			// instance of com.sun.faces.application.ViewHandlerResponseWrapper. 
				...
				if (portletPhase == Bridge.PortletPhase.HEADER_PHASE) {
					portletResponse = new HttpServletResponseHeaderAdapter((HeaderResponse) portletResponse,
							httpServletResponse);
				}
	 ...
	 *
	 * <p>Try#1: Get the viewId from the {@link Bridge#VIEW_ID} (com.liferay.portlet.faces.viewId) request attribute. As
	 ...
	 *
	 * <p>Try#2: Get the viewId from the {@link Bridge#VIEW_PATH} (com.liferay.portlet.faces.viewPath) request attribute. As
	 * described in sections 3.4 and 4.2.5 of the bridge spec, this attribute is set by the {@link GenericFacesPortlet}
	...
	protected String getFacesViewIdAndQueryString() throws BridgeDefaultViewNotSpecifiedException,
		...
		if (viewIdAndQueryString == null) {
			...
			// Try#1: Get the viewId the "com.liferay.portlet.faces.viewId" request attribute.
			...
			if (viewIdAndQueryString == null) {
				...
				// Try#2: Get the viewId from the "com.liferay.portlet.faces.viewPath" request attribute.
				...
				logger.debug("com.liferay.portlet.faces.viewId=[{0}]", viewIdAndQueryString);
			}
		}
