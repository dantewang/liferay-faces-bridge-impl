...
public class BridgeURLResourceImpl extends BridgeURLBase {
	...
	public BridgeURLResourceImpl(FacesContext facesContext, String uri, String contextPath, String namespace,
		...
		// If the "com.liferay.portlet.faces.ViewLink" parameter is found and set to "true", then
		...
		if (BooleanHelper.isTrueToken(viewLinkParam)) {
			...
			// "com.liferay.portlet.faces.ViewLink" parameter must be removed from the URI's query-string.
			bridgeURI.removeParameter(Bridge.VIEW_LINK);

			// If the "com.liferay.portlet.faces.BackLink" parameter is found in the URI query-string, then replace it's value
		...
		// Otherwise, if the URI is hierarchical and targets an external resource, then
			...
			// If the "com.liferay.portlet.faces.BackLink" parameter is found, then replace it's value with an encoded URL
		...
		// Otherwise, if the URI is hierarchical and targets a resource internal to this application, then
			...
			// If the "com.liferay.portlet.faces.InProtocolResourceLink" parameter is found, then
			if ((bridgeURI.getParameter(Bridge.IN_PROTOCOL_RESOURCE_LINK) != null)) {

				// Since an in-protocol-resource URL must be a ResourceURL, the "com.liferay.portlet.faces.PortletMode" and
				// "com.liferay.portlet.faces.WindowState" parameters must be removed from the URI query-string (if present)
	...
	public BaseURL toBaseURL(FacesContext facesContext) throws MalformedURLException {
		...
		if (bridgeURI.isOpaque()) {
			...
			if (bridgeURI.isPortletScheme()) {
				...
				// According to the Bridge Spec, the "com.liferay.portlet.faces.Secure" parameter must not be "carried
		...
		// Otherwise, if the URL originally contained the "com.liferay.portlet.faces.ViewLink" which represents navigation
